...
class Cli( Kanashi, Util ):
	...
	#[Profile.profileEdges( Profile user )]
	def profileEdges( self, user:Profile ) -> None:
		options = {
			"edge.followers": "Followers",
			"edge.following": "Followings",
			"edge.mutual": "Mutual Followed By",
			"edge.video": "Felix Video Timeline",
			"edge.timeline": "Owner to Timeline Media",
			"edge.saved": "Media Saved",
			"edge.story": "Media Story",
			"edge.highlight": "Media Highlight",
			"edge.collection": "Media Collections",
			"back": "Back"
		}
		self.output( self, [ *user.outputs, [ val for val in options.values() ] ] )
		keys = [ key for key in opts.keys() ]
		next = self.input( None, number=True, default=[ 1+ i for i in range( len( options ) ) ] )
		match keys[( next -1 )]:
			case "edge.followers":
				pass
			case "edge.following":
	...
	def profileOptions( self, user:Profile ) -> None:
		...
		match keys[( next -1 )]:
			...
			case "user.profile-edges":
				self.profileEdges( user )
